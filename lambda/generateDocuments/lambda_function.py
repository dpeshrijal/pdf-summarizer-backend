...
def lambda_handler(event, context):
    try:
        ...
        if not job_description or not file_id:
            return {
                "statusCode": 400,
                "headers": CORS_HEADERS, # <-- CORRECTED
                "body": json.dumps({"error": "jobDescription and fileId are required"})
            }

        # --- (RAG Pipeline: Embed, Query, Prompt) ---
        ...
        # This is a successful response, so it needs the CORS headers
        return {
            "statusCode": 200,
            "headers": CORS_HEADERS, # <-- CORRECTED (and removed other headers handled by frontend now)
            "body": pdf_base64,
        ...
        # This is an error response, so it ALSO needs the CORS headers
        return {
            "statusCode": 500,
